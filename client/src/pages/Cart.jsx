                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <button className="cursor-pointer mx-auto">
                            <img  onClick={()=>removeFromCart(product._id)} src={RemoveIcon} alt="removeIcon" className="inline-block w-6 h-6"/>
                        </button>
                    </div>)
                )}

                <button className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
                    <img onClick={()=>{navigate("/products");scrollTo(0,0)}} src={RightArrow} alt="arrow" className="group-hover:-translate-x-1 transition w-3"/>
                    Continue Shopping
                </button>
